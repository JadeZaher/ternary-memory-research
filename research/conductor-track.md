...
| **Gate 15** | Track D-4: Frontier Scale Multi-Corpus Pretraining | **NaviTrit-10M validated** (11.93M params, 2.3MB packed footprint) on balanced 7.85M token multi-corpus (TinyStories + GSM8K Math + Python Code). Trained 2,500 steps in **306.81s (5.11 min)** on RTX 4060. **Val loss 0.2214 vs 0.2231 monotonic baseline (PPL 1.25)**, maintaining 50% attention ratio. Discovered non-monotonic trajectory: `FFN 3 -> Attn 1 -> FFN 0 -> Attn 1 -> Attn 2 -> FFN 0`. Generation audit confirmed multi-step arithmetic chain-of-thought and structured Python code completion. Checkpoint: `outputs/checkpoints/navitrit-10m-trained.pt`. Ledgers: `outputs/navitrit-10m-results.json`, `outputs/navitrit-10m-generation-audit.json`. 100M configuration prepared in `experiments/frontier_scaling/`. | **PASSED** |
