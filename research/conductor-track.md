...
| **Gate 15** | Track D-4: Frontier Scale Multi-Corpus Pretraining | **NaviTrit-10M & 100M validated** on balanced 7.85M token multi-corpus (TinyStories + GSM8K Math + Python Code). **10M scale (11.93M params, 2.3MB packed):** Trained in 306.81s (5.11 min) on RTX 4060; **val loss 0.2214 vs 0.2231 monotonic baseline (PPL 1.25)**, 50% attention ratio, non-monotonic trajectory `[7, 2, 1, 2, 4, 1]`. **100M scale (129.70M params, 27.0MB packed):** Trained in 440.24s (7.34 min) on RTX 4060, peak VRAM 2.8 GB; **val loss 0.3420 vs 0.2915 baseline (PPL 1.41 vs 1.34)**. Uncovered scaling attractor dynamic: large FFN capacity ($768 \times 2048$) creates strong representational gravity towards deep FFN tiles (`node 15` / $\text{FFN}_7$), suppressing attention ratio to 17.55% and requiring scale-adaptive diversity weighting. Checkpoints: `outputs/checkpoints/navitrit-10m-trained.pt`, `outputs/checkpoints/navitrit-100m-trained.pt`. Ledgers: `outputs/navitrit-10m-results.json`, `outputs/navitrit-100m-results.json`. | **PASSED** |
